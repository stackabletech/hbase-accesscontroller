 ...
 *
 * <p>TODO: There is room for further performance optimization here. Calling AuthManager.authorize()
 ...
 *
 * <p>We should aim to use this _only_ when access to the requested column families is not granted
 * at the column family levels. If table or column family access succeeds, then there is no need to
 * impose the overhead of this filter.
 */
@InterfaceAudience.Private
class OpenPolicyAgentAccessControlFilter extends FilterBase {
  ...
  OpenPolicyAgentAccessControlFilter(
      AuthManager mgr,
      User ugi,
      TableName tableName,
      Strategy strategy,
  ...
  public ReturnCode filterCell(final Cell cell) {
    ...
    if (prevFam.getBytes() == null
        || !(PrivateCellUtil.matchingFamily(
            cell, prevFam.getBytes(), prevFam.getOffset(), prevFam.getLength()))) {
    ...
    if (prevQual.getBytes() == null
        || !(PrivateCellUtil.matchingQualifier(
            cell, prevQual.getBytes(), prevQual.getOffset(), prevQual.getLength()))) {
  ...
  public static OpenPolicyAgentAccessControlFilter parseFrom(final byte[] pbBytes)
  ...
  public boolean equals(Object obj) {
    if (!(obj instanceof OpenPolicyAgentAccessControlFilter)) {
      return false;
    }
    if (this == obj) {
      return true;
    }
    OpenPolicyAgentAccessControlFilter f = (OpenPolicyAgentAccessControlFilter) obj;
  ...
  public int hashCode() {
    return Objects.hash(
        this.authManager, this.table, this.strategy, this.user, this.cfVsMaxVersions);
  }
}
